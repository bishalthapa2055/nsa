...
const sendOtpArenaController = async(req :Request , res : Response ) =>{
    try {
        ...
        if(isExistUser){
            throw new BadRequestError("Phone Number Already registered as Player.")
        }

        const isExistArena = await Arena.findOne({phone_number});

        if(isExistArena){
            throw new BadRequestError("Phone number already registered as Futsal Arena . Proceed to login")
        }
...
export  { sendOtpArenaController  as sendOtpArenaControllerHandler }